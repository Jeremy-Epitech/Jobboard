...
//add Candidature
router.post('/candid', (request, response) => {
    let ads = request.body;
    console.log(request.body);
    var sql = "SET @idPeople = ?;SET @idAd = ?;SET @message = ?;INSERT INTO candidature(idPeople, idAd, message) values (@idPeople, @idAd, @message);"
    mysqlConnection.query(sql, [ads.idPeople, ads.idAd, ads.message], (err, rows, fields) => {
        if (!err) {
            response.send('success')
        } else {
            response.send(err);
